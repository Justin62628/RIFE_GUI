...
class InterpWorkFlow:
    ...
    def nvidia_run(self):
        ...
        while True:
            ...
            pbar.set_description(
                f"Process at 区块-{rsq[0]:0>3d}, 已渲染帧 {rsq[3]:0>6d}, 当前帧 {now_frame:0>6d}, 最近的转场 {recent_scene:0>6d}")
    ...
    def amd_whole_run(self):
        ...
        while True:
            ...
            pbar.set_description(
                f"Process at Step 1: 抽帧： 当前帧 {now_frame:0>6d}, 最近的转场 {recent_scene:0>6d}")
            pbar.update(now_frame - previous_cnt)
            previous_cnt = now_frame
            ...
            if img1 is None:
                ...
                while self.rife_core.is_alive():
                    ...
                    pbar.set_description(
                        f"Process at Step 2: 补帧 第 {self.rife_core.supervise_data['now_dir']} 轮")
                ...
                for frame_path in interp_list:
                    ...
                    if str(interp_cnt) in interp_scene_list:
                        ...
                        video_renderer.writeFrame(frame_ioer.read_frame(frame_path))
                    pbar.set_description(
                        f"Process at Step 3: 渲染帧 {interp_cnt:0>6d}")
                    pbar.update(1)
    ...
    def amd_run(self):
        ...
        while True:
            ...
            rsq = self.render_info_pipe["rendering"]  # render status quo
            pbar.set_description(
                f"Process at Step 1: 渲染区块 {rsq[0]}, 抽帧 {now_frame:0>8d}, 最近的转场 {recent_scene:0>8d}")
            pbar.update(now_frame - previous_cnt)
            previous_cnt = now_frame
            ...
            if now_frame % (self.render_gap / (2 ** self.exp)) == 0 or img1 is None:
                ...
                while self.rife_core.is_alive():
                    ...
                    rsq = self.render_info_pipe["rendering"]  # render status quo
                    pbar.set_description(
                        f"Process at Step 2: 渲染区块 {rsq[0]}, 第 {self.rife_core.supervise_data['now_dir']} 轮补帧")
                ...
                for interp_frame in interp_list:
                    ...
                    """(chunk_cnt, start_frame, end_frame, frame_cnt)"""
                    pbar.set_description(
                        f"Process at Step 3: 渲染区块 {rsq[0]}, 输入帧 {interp_cnt:0>6d}, 渲染帧 {int(rsq[3]):0>6d}")
